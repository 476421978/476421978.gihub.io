<template>
  <div>
    <div class="sub-search-com">
      <el-form ref="formRef" :model="comGetForm" size="large" :inline="true">
        <el-row class="search-box" type="flex" :gutter="24">
          <template v-for="(item, index) in searchData">
            <el-col :span="item.span || 6">
              <el-form-item :label="`${item.title}`" :prop="item.fName" :rules="item.rules ? item.rules : []" style="width: 100%">
                <t-input v-if="item.comType === 'input'" :id="item.id" v-model="item.fValue" :placeholder="item.placeholder" />
                <t-select v-if="item.comType === 'select'" :id="item.id" v-model="item.fValue" :options="item.options" @on-select-change="onSelectChange" />
                <t-area v-if="item.comType === 'area'" :areaArr="item.areaArr" :value="item.fValue" @get-cascade="getCascade" />
                <t-cascader v-if="item.comType === 'cascader'" :id="item.id" :value="item.fValue" :options="item.options" v-model="item.fValue" />
                <t-date-picker
